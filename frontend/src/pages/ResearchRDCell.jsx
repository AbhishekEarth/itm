    ...
    <div className="min-h-screen bg-[#fbf7f2] dark:bg-[#020617]">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-16 md:py-20">
        <div className="grid md:grid-cols-2 gap-4">
          <div className="relative overflow-hidden bg-white rounded-3xl border border-rose-50 shadow-sm p-8">
            ...
            <p className="text-base text-gray-700 leading-relaxed font-medium italic">&ldquo;{VISION}&rdquo;</p>
          </div>
          <div className="relative overflow-hidden bg-white rounded-3xl border border-rose-50 shadow-sm p-8">
            ...
            <div className="text-[10px] font-black uppercase tracking-[0.3em] text-amber-700 mb-2">R&amp;D Mission</div>
            <p className="text-base text-gray-700 leading-relaxed font-medium italic">&ldquo;{MISSION}&rdquo;</p>
          </div>
        </div>
      </section>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {KEY_FEATURES.map((f, i) => (
              <motion.div key={f.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} whileHover={{ y: -4 }}
                className="bg-gradient-to-br from-white to-rose-50/40 border border-rose-50 rounded-3xl p-5 hover:shadow-xl transition-shadow">
                <div className="text-3xl mb-3">{f.icon}</div>
                <h4 className="font-black text-sm text-[#1a0606] tracking-tight mb-2 leading-snug">{f.title}</h4>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {RESEARCH_AREAS.map((a, i) => (
            <button key={a.name} onClick={() => setActiveArea(i)}
              className={`group relative overflow-hidden text-left rounded-2xl border-2 transition-all p-4 ${i === activeArea ? "border-[#800000] shadow-xl bg-white" : "border-rose-50 bg-white hover:border-rose-200"}`}>
              <div className="text-3xl mb-2">{a.icon}</div>
              <h4 className="font-black text-xs tracking-tight text-[#1a0606] leading-snug line-clamp-2">{a.name}</h4>
            </button>
          ))}
        </div>
      </section>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {DEPT_RESEARCH.map((d, i) => (
              <motion.div key={d.short} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} whileHover={{ y: -4 }}
                className="group relative overflow-hidden bg-white border border-rose-50 rounded-3xl shadow-sm hover:shadow-xl transition-shadow">
                ...
                <div className="p-6">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <h3 className="font-black text-sm text-[#1a0606] tracking-tight leading-snug">{d.dept}</h3>
                    </div>
                  </div>
                  <div className="space-y-3">
                    <div>
                      <div className="text-[9px] uppercase tracking-widest font-black text-[#800000] mb-1">Research focus</div>
                      <p className="text-xs text-gray-700 leading-relaxed font-medium">{d.research}</p>
                    </div>
                    <div>
                      <div className="text-[9px] uppercase tracking-widest font-black text-amber-700 mb-1">Patents / IPR areas</div>
                      <p className="text-xs text-gray-700 leading-relaxed font-medium">{d.patents}</p>
                    </div>
                  </div>
                </div>
              </motion.div>
          ...
          {pubTab === "papers" ? (
            <motion.div key="papers" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {PUBLICATIONS.map((p, i) => (
                <a key={p.year} href={p.url} target="_blank" rel="noreferrer" className={`group relative overflow-hidden rounded-3xl p-6 hover:shadow-2xl transition-shadow ${i === 0 ? "bg-gradient-to-br from-[#3e0202] to-[#800000] text-white" : "bg-white border border-rose-50"}`}>
            ...
            <motion.div key="books" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {BOOKS.map((b, i) => (
                <a key={b.year} href={b.url} target="_blank" rel="noreferrer" className={`group relative overflow-hidden rounded-3xl p-6 hover:shadow-2xl transition-shadow ${i === 0 ? "bg-gradient-to-br from-[#3e0202] to-[#800000] text-white" : "bg-white border border-rose-50"}`}>
                  <div className="flex items-start justify-between mb-3">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
            {DOCUMENTS.map((d, i) => (
              <motion.a key={d.title} href={d.url} target="_blank" rel="noreferrer" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }} whileHover={{ y: -4 }}
                className="group relative overflow-hidden bg-gradient-to-br from-white to-rose-50/40 border border-rose-50 rounded-3xl p-6 hover:shadow-xl transition-shadow">
                <div className="w-10 h-10 rounded-xl bg-rose-50 text-[#800000] flex items-center justify-center mb-4"><d.icon size={18} /></div>
                <h4 className="font-black text-sm text-[#1a0606] tracking-tight mb-2 leading-snug">{d.title}</h4>
                <p className="text-xs text-gray-600 leading-relaxed font-medium mb-4">{d.desc}</p>
