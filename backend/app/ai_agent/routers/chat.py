...
async def _stream_response(message: str, session_id: str):
    ...
    Three event types are emitted to the client:
      * `{"token": "..."}` — incremental Markdown chunk of the answer body
      * `{"suggestions": [{"label", "path"}, ...]}` — page links to chip-render
      * `{"done": true, "session_id": "..."}` — terminator
    """
    try:
        async for token in assistant.stream_chat(message, session_id):
            yield f"data: {json.dumps({'token': token})}\n\n"

        # Emit suggested ITM pages as a structured event so the frontend can
        # render them as coloured chips outside the message bubble.
        suggestions = assistant.suggest_pages(message)
        if suggestions:
