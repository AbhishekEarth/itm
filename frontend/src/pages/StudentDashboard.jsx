...
export default function StudentDashboard() {
  ...
  const INFO_CARDS = [
    { icon: Hash, label: 'Enrollment No.', value: user.enrollment_no, accent: 'bg-indigo-50 text-indigo-600' },
    { icon: Building2, label: 'Department', value: user.department, accent: 'bg-rose-50 text-[#800000]' },
    { icon: Layers, label: 'Year', value: user.year, accent: 'bg-amber-50 text-amber-700' },
  ...
  const QUICK_LINKS = [
    ...
    { label: 'My Department', desc: `Explore ${user.department} dept.`, to: deptLink, accent: 'from-indigo-500 to-violet-700' },
      ...
      <div className="max-w-5xl mx-auto px-6 py-10 space-y-8">
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={0}
          ...
          <div className="flex-1">
            ...
            <p className="text-gray-500 text-sm mt-1">{user.year} Year · {user.department} · Batch {user.batch}</p>
          </div>
        </motion.div>
