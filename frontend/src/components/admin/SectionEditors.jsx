...
export function HeroEditor({ value = {}, onChange, media }) {
  ...
  const setCta = (slot, field, val) => {
    const current = v[slot] || {};
    onChange({ ...v, [slot]: { ...current, [field]: val } });
  };
  const primary = v.primary_cta || {};
  const secondary = v.secondary_cta || {};
  return (
    <div className="grid sm:grid-cols-2 gap-3">
      <label className="sm:col-span-2">
        <span className={cls.label}>Headline</span>
        <input className={cls.field} value={v.headline || v.title || ''} onChange={(e) => set('headline', e.target.value)} />
      </label>
      <label className="sm:col-span-2">
        <span className={cls.label}>Subheading</span>
        <input className={cls.field} value={v.subhead || v.subtitle || ''} onChange={(e) => set('subhead', e.target.value)} />
      </label>

      {/* Primary CTA — e.g. "Apply Now" */}
      <label>
        <span className={cls.label}>Primary CTA label</span>
        <input className={cls.field} value={primary.label || ''} onChange={(e) => setCta('primary_cta', 'label', e.target.value)} />
      </label>
      <label>
        <span className={cls.label}>Primary CTA link</span>
        <input className={cls.field} value={primary.href || ''} onChange={(e) => setCta('primary_cta', 'href', e.target.value)} placeholder="/admissions/how-to-apply" />
      </label>

      {/* Secondary CTA — e.g. "Watch Tour" / "Learn More" */}
      <label>
        <span className={cls.label}>Secondary CTA label</span>
        <input className={cls.field} value={secondary.label || ''} onChange={(e) => setCta('secondary_cta', 'label', e.target.value)} />
      </label>
      <label>
        <span className={cls.label}>Secondary CTA link</span>
        <input className={cls.field} value={secondary.href || ''} onChange={(e) => setCta('secondary_cta', 'href', e.target.value)} placeholder="https://… or /route" />
      </label>
