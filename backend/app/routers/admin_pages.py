...
@router.post("", response_model=AdminPageListItem, status_code=201)
def create_admin_page(
    body: AdminPageCreate,
    db: Session = Depends(get_db),
    actor: User = Depends(get_current_user),
):
    """Create a brand-new public page from the admin panel.

    Path must be unique (e.g. ``/student-life``) and start with ``/``.
    Defaults the new row to ``status='published'`` so it shows up on the public
    site immediately; admins can switch to draft via the editor.
    """
    from sqlalchemy.exc import IntegrityError
    ...
    existing = db.scalar(select(Page).where((Page.key == key) | (Page.path == path)))
    if existing:
        raise ConflictError(f"A page with key '{existing.key}' or path '{existing.path}' already exists")
    ...
    try:
        ...
        db.rollback()
        raise ConflictError("Page key or path conflicts with another record") from e
    db.refresh(page)
...
def delete_admin_page(
    ...
    """Permanently delete a page (and its sections, via cascade)."""
